...
| `test_netwatch_rhythm.py` | Loss that comes back on a schedule: 10-, 5- and 15-minute jobs found at their real period (not a divisor), a drifting timer told from a clock-driven job, random loss and a single long outage left alone — then whole synthetic captures: the finding, where it breaks, the score, the report section and chart, and that netwatch's own speed tests and traceroutes are not blamed on the line. |

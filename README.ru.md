...
| [`backup.sh`](#backupsh) | Linux | Архивирует указанные каталоги и Docker-тома, проверяет каждый архив, чистит старые и восстанавливает — по умолчанию во временный каталог, а не поверх живой системы. |
...
Резервная копия, которую никто не восстанавливал, — это слух. Здесь это обычный поток `tar` (zstd,
если он есть, иначе gzip), поэтому восстановление никогда не зависит от наличия самого скрипта —
достаточно `tar -xf`. После записи архив перечитывается целиком, рядом сохраняется SHA-256 и
манифест: что вошло, какого размера, сколько заняло и **была ли копия консистентной**.

Именно консистентность большинство скриптов тихо портит: копирование базы во время записи может
поймать файл в разорванном состоянии. С `--stop <каталог-compose>` стек останавливается на время
копирования и запускается снова; без него копия «горячая», и об этом сообщают и вывод, и манифест.

`--restore` по умолчанию распаковывает во **временный каталог** — живая система не трогается, пока
вы не посмотрите содержимое и не повторите с `--in-place` (там нужно ввести `RESTORE`).
...
> 💡 Полный цикл проверяется, а не предполагается: тесты создают копию в контейнере, удаляют
> источник, восстанавливают и сверяют контрольные суммы — а затем обрезают архив, чтобы убедиться,
> что проверка целостности это замечает.

#!/usr/bin/env bash
#
# netwatch.sh — launcher for the continuous internet quality monitor.
#
# toolkit-name: netwatch — internet quality monitor
# toolkit-kind: tool
# toolkit-category: Diagnostics
# toolkit-summary: Watches the connection over time, stores every sample in SQLite and writes a report with charts and a verdict.
# toolkit-os: any
# toolkit-root: optional
# toolkit-needs: python3
# toolkit-optional: ping, traceroute
# toolkit-preview: --quick --yes --no-tui
# toolkit-docs: linux/netwatch/README.md
# toolkit-order: 30
# toolkit-arg: --duration | How long to monitor: 30m, 2h, 0 = until you stop it | text
# toolkit-arg: --plan | Your subscribed speed in Mbps, for the verdict | number
# toolkit-arg: --out | Directory to write the report into | path
#
# Ensures Python 3 is present (installing it via the distro package manager if
# needed) and then runs netwatch.py, forwarding all arguments to it.
#
#   ./netwatch.sh                              interactive TUI menu
#   ./netwatch.sh --quick                       one-shot 60-90 s diagnostic
#   ./netwatch.sh --duration 2h --plan 100 --yes
#   ./netwatch.sh --analyze ./netwatch-20260821-120000
#   ./netwatch.sh --help                        full option list
#
# Everything is measured with the Python standard library only — no pip packages,
# and no external binaries are required (ping/traceroute are used when present,
# otherwise netwatch does ICMP and traceroute itself over a socket).
#
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)"
APP="$SCRIPT_DIR/netwatch.py"

# ---- pretty output ----
if [ -t 1 ]; then
    C_I=$'\033[1;34m'; C_W=$'\033[1;33m'; C_E=$'\033[1;31m'; C_0=$'\033[0m'
else
    C_I=''; C_W=''; C_E=''; C_0=''
fi
info() { printf '%s[*]%s %s\n' "$C_I" "$C_0" "$*"; }
warn() { printf '%s[!]%s %s\n' "$C_W" "$C_0" "$*" >&2; }
die()  { printf '%s[x]%s %s\n' "$C_E" "$C_0" "$*" >&2; exit 1; }

[ -f "$APP" ] || die "Cannot find netwatch.py next to this script ($APP)."

# ---- privilege helper (only needed to install python) ----
if [ "$(id -u)" -eq 0 ]; then SUDO=""; else SUDO="sudo"; fi

find_python() {
    for c in python3 python; do
        if command -v "$c" >/dev/null 2>&1; then
            "$c" -c 'import sys; raise SystemExit(0 if sys.version_info[:2] >= (3, 8) else 1)' \
                >/dev/null 2>&1 && { echo "$c"; return 0; }
        fi
    done
    return 1
}

install_python() {
    warn "Python 3 not found — attempting to install it."
    command -v sudo >/dev/null 2>&1 || [ "$(id -u)" -eq 0 ] || die "Need root/sudo to install Python 3."
    if [ -r /etc/os-release ]; then . /etc/os-release; fi
    case "${ID:-}${ID_LIKE:-}" in
        *debian*|*ubuntu*) $SUDO apt-get update && $SUDO DEBIAN_FRONTEND=noninteractive apt-get install -y python3 ;;
        *fedora*|*rhel*|*centos*) $SUDO dnf -y install python3 ;;
        *)
            if   command -v apt-get >/dev/null 2>&1; then $SUDO apt-get update && $SUDO apt-get install -y python3
            elif command -v dnf     >/dev/null 2>&1; then $SUDO dnf -y install python3
            elif command -v yum     >/dev/null 2>&1; then $SUDO yum -y install python3
            elif command -v pacman  >/dev/null 2>&1; then $SUDO pacman -Sy --noconfirm python
            elif command -v apk     >/dev/null 2>&1; then $SUDO apk add --no-cache python3
            else die "Could not detect a package manager. Install Python 3 manually."
            fi ;;
    esac
}

PY="$(find_python || true)"
if [ -z "${PY:-}" ]; then
    install_python
    PY="$(find_python || true)"
    [ -n "${PY:-}" ] || die "Python 3.8+ still not available after install."
fi

# ---- ICMP availability hint -------------------------------------------------
# netwatch prefers unprivileged ICMP (SOCK_DGRAM). Most distros allow it; where the
# kernel does not, it silently falls back to the ping binary and then to TCP probes,
# which measure connect() time rather than true round-trip time.
if [ "$(id -u)" -ne 0 ] && [ -r /proc/sys/net/ipv4/ping_group_range ]; then
    read -r LO HI < /proc/sys/net/ipv4/ping_group_range || true
    GID="$(id -g)"
    if [ "${LO:-1}" -gt "${HI:-0}" ] || [ "$GID" -lt "${LO:-1}" ] || [ "$GID" -gt "${HI:-0}" ]; then
        if ! command -v ping >/dev/null 2>&1; then
            warn "Unprivileged ICMP is disabled and 'ping' is missing — probes will fall back to TCP."
            warn "For real ICMP timing run with sudo, or:  sudo sysctl -w net.ipv4.ping_group_range='0 2147483647'"
        fi
    fi
fi

exec "$PY" "$APP" "$@"
